...
function UserDash() {
  ...
  const { data: m } = useQuery({
    queryKey: ["user", "metrics", user.empId],
    queryFn: () => getUserMetrics(user.empId),
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Your ticket activity at a glance."
      />
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <MetricCard
          ...
          hint={statusFilter === null ? "Showing all" : "Click to clear filter"}
        />
        <MetricCard
          label="Open"
          value={m?.open ?? "—"}
