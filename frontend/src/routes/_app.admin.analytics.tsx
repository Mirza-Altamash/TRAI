...
function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Analytics"
        subtitle="Cross-division ticket performance and workload distribution."
      />

      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <MetricCard
          label="Avg Resolution Time"
          value={`${a.avgResolutionDays} days`}
          icon={Timer}
          accent="primary"
        />
        <MetricCard
          label="Avg Closure Time"
          value={`${a.avgClosureDays} days`}
          icon={TimerReset}
          accent="info"
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={a.byPriority} dataKey="count" nameKey="name" outerRadius={90} label>
                {a.byPriority.map((_: any, i: number) => (
        ...
        <ChartCard
          title="Open vs Resolved (Monthly)"
          customTotalText={`Total Open: ${a.monthly.reduce((sum: number, item: { open?: number }) => sum + (item.open || 0), 0)} | Total Resolved: ${a.monthly.reduce((sum: number, item: { resolved?: number }) => sum + (item.resolved || 0), 0)}`}
        >
          <ResponsiveContainer width="100%" height={260}>
        ...
        {totals && totals.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-3 border-t border-border/60 justify-center">
            {totals.map((t) => (
              <span
                key={t.name}
                className="inline-flex items-center gap-1 rounded-full bg-secondary/15 border border-secondary/30 px-2.5 py-0.5 text-xs font-medium text-foreground"
              >
                <span className="font-semibold text-muted-foreground mr-1">{t.name}:</span>{" "}
                {t.count}
              </span>
            ))}
          </div>
